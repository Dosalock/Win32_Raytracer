 ...
 */
inline __device__ float4 make_float4(const float &x, const float &y, const float &z)
{
...
{
	return make_float4(v.x*-1,
					   v.y*-1,
					   v.z*-1);
}

inline __device__ double len(const float4 &v)
{
...
inline __device__ float4 calc_forward_euler_angle(const float &yaw, const float &pitch)
{
	...
	return make_float4(cos_pitch*sin_yaw,
					   sin_pitch,
					   cos_pitch*cos_yaw);
}
