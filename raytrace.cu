...
__device__ float CalcLight(const float4& P, const float4& N, const float4& V, const int& s)
{
	float intensity = 0.0;
	float t_max = 0;
	float4 L = {};
	...
	for (int i = 0; i < sizeof(cuda_lights) / sizeof(Light); i++)
	{
		...
		else
		{
			...
			L = L;
			auto [closest_sphere, closest_t] = ClosestIntersection(P, L, 0.00001, t_max);
			...
			if (n_dot_l > 0)
			{
				intensity += cuda_lights[i].intensity * n_dot_l / (len(N) * len(L));
			}
			...
			if (s != -1)
			{
				...
				if (r_dot_v > 0)
				{
					intensity += cuda_lights[i].intensity * pow(r_dot_v / (len(R) * (len(V))), s);
				}
...
__device__ float IntersectRaySphere(const float4& O, const float4& D, const Sphere& sphere, const double& dDot)
{
	float4 CO = {};
	CO = O - sphere.center;
	...
	float t = (-b - __fsqrt_rd(discr)) / (2 * a);		// Minimize compute only go for 1 root;
...
__device__ std::tuple<Sphere*, float> ClosestIntersection(const float4& O, const float4& D, const double& t_min, const double& t_max)
{
	...
	return std::tuple<Sphere*, float>(closest_sphere, closest_t);
}

__device__ COLORREF TraceRay(const float4& O, const float4& D, const float& t_min, const float& t_max, const int& recursionDepth)
{
	float4 N = {};
	float4 P = {};
	float4 R = {};
...
__device__ float4 CanvasToViewPort(const int &x, const int &y)
{
	...
	float fovMod = 1;
	float viewportX = (x - WIDTH / 2.0) * ((1.0 * fovMod) / WIDTH) * aspectRatio;
	float viewportY = -(y - HEIGHT/ 2.0) * ((1.0 * fovMod) / HEIGHT); // Flip Y to match 3D space orientation

	return make_float4(viewportX, viewportY, 1, 0);  // Z=1 for perspective projection
}
...
__global__ void cuda_Draw(BYTE *pLpvBits, Camera &cam)
{
	...
	D = normalize(apply_camera_rotation(cam.position, cam.yaw, cam.pitch, cam.roll));
